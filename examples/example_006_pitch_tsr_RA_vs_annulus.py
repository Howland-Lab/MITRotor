...
FN_PITCH_TSR = Path("pitch_tsr_RA_vs_AA.csv")


def func(x):
    method, pitch, tsr, yaw = x
    relax = 0.5 if method == "rotoraveragedunified" else 0.25
...
if __name__ == "__main__":
    main()
    methods = ["rotoraveragedunified", "unified"]
    ...
    (
        pitch_best_rotoraveragedunified,
        tsr_best_rotoraveragedunified,
        Cp_best_rotoraveragedunified,
    ) = (
        df.filter(pl.col("method") == "rotoraveragedunified")
        .filter(pl.col("Cp") == pl.col("Cp").max())
        .select("pitch", "tsr", "Cp")
        .to_numpy()[0]
    )
    ...
    marker_properties = {
        ...
        "optimal_rotoraveragedunified": dict(
            label="Optimal\n(rotoraveragedunified (2020))",
            marker="*",
            ms=5,
            c="tab:orange",
            ls="",
    ...
    df_piv_rotoraveragedunified_Cp = (
        df.filter(pl.col("method") == "rotoraveragedunified")
        .pivot(index="tsr", columns="pitch", values="Cp_filt", aggregate_function=None)
        .fill_nan(None)
        .interpolate()
    ...
    df_piv_rotoraveragedunified_Ct = (
        df.filter(pl.col("method") == "rotoraveragedunified")
        .pivot(index="tsr", columns="pitch", values="Ct", aggregate_function=None)
        .fill_nan(None)
        .interpolate()
    ...
    Cp_rotoraveragedunified = df_piv_rotoraveragedunified_Cp.to_numpy()[:, 1:]
    Ct_rotoraveragedunified = df_piv_rotoraveragedunified_Ct.to_numpy()[:, 1:]

    mask_unified = np.isnan(Cp_unified)
    mask_rotoraveragedunified = np.isnan(Cp_rotoraveragedunified)
    Cp_unified[mask_unified] = 0
    Cp_rotoraveragedunified[mask_rotoraveragedunified] = 0
    Ct_unified[mask_unified] = 0
    Ct_rotoraveragedunified[mask_rotoraveragedunified] = 0
    Ct_rotoraveragedunified[(Ct_unified > 1.25)] = 1.51
    ...
    axes[1].contourf(pitch, tsr, Cp_rotoraveragedunified, levels=levels, cmap="viridis")
    CS = axes[1].contour(pitch, tsr, Cp_rotoraveragedunified, levels=levels, colors="k")
    axes[1].clabel(CS, inline=True, fontsize=10)

    # diff
    Cp_diff = np.abs(Cp_unified - Cp_rotoraveragedunified)
    ...
    axes[1].set_title("$C_P$ (rotoraveragedunified (2020))", fontsize="small")
